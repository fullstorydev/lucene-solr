...
public class TestLockTree extends SolrTestCaseJ4 {
  ...
  public void testLocks() throws Exception {
    ...
    List<ImmutableSet<String>> orderOfExecution = Arrays.asList(
    ...
    for (int counter = 0; counter < orderOfExecution.size(); counter++) {
      ...
      for (int i = 0; i < operations.size(); i++) {
        ...
        final Lock lock = session.lock(operation.getKey(), operation.getValue());
        if (lock != null) {
  ...
  private Runnable getRunnable(final List<Pair<CollectionAction, List<String>>> completedOps, final Pair<CollectionAction,
      List<String>> operation, final List<Lock> locks, final Lock lock) {
    return new Runnable() {
      @Override
      public void run() {
        try {
