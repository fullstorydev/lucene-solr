...
public class LockTree {
  ...
  private class LockImpl implements Lock {
    ...
    public String toString() {
      return StrUtils.join(node.constructPath(new LinkedList<String>()), '/');
    }
  }
  ...
  private class Node {
    ...
    void clear() {
      if (myLock != null) {
        LOG.warn("lock_is_leaked at" + constructPath(new LinkedList<String>()));
        myLock = null;
      }
      for (Node node : children.values()) node.clear();
    }
  }
  static final Lock FREELOCK = new Lock() {
    @Override
    public void unlock() {
    }
  };
